...
enum DeploymentCmd {
    ...
    Create {
        ...
        #[arg(long, default_value = "immediate")] strategy: String,
        #[arg(long)] canary_percent: Option<u8>,
    },
    ...
    pub async fn run(self, client: &ApiClient, fmt: &OutputFormat) -> Result<()> {
        match self.cmd {
            DeploymentCmd::List { tenant } => {
                let items: Vec<DeploymentRow> =
                    client.get(&format!("/v1/tenants/{tenant}/deployments")).await?;
                print_table(&items, fmt);
            }
            DeploymentCmd::Create { tenant, group, agent, version, strategy, canary_percent } => {
                ...
                let item: DeploymentRow =
                    client.post(&format!("/v1/tenants/{tenant}/deployments"), body).await?;
                println!("Created deployment: {}", item.policy_id);
                print_item(&item, fmt);
            }
            DeploymentCmd::Rollback { deployment_id, tenant } => {
